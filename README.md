...
> 위 배포 링크는 **프론트엔드(Vercel, 정적 배포)** 기준이며, 백엔드는 로컬에서 별도로 실행해야 실제 데이터 연동이 동작합니다. 자세한 내용은 [시작하기](#-시작하기)를 참고하세요.
...
| Deploy | Vercel (Frontend) | SPA, `vercel.json`으로 client-side 라우팅 rewrite 처리 |
...
- **Frontend**: Vercel (Root Directory `frontend`, Build Command `npm run build`, Output Directory `dist`) → **[tech4-good-care-ez-mu.vercel.app](https://tech4-good-care-ez-mu.vercel.app/)**
- **Backend**: 별도 배포 없이 로컬 실행 기준. 배포된 프론트엔드에서 실 데이터를 보려면 백엔드를 로컬 또는 별도 서버에 띄우고 API base URL을 연결해야 합니다.
